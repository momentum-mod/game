...
float2 parallaxCorrect(float2 texCoords, float3 viewRelativeDir, sampler depthMap, float parallaxDepth, float parallaxCenter)
{
    static const int numLayers = 50;
    float layerDepth = 1.0 / numLayers;
    float currentLayerDepth = 0.0;
    float2 P = viewRelativeDir.xy / viewRelativeDir.z * parallaxDepth;
    float2 deltaTexCoords = P / numLayers;
    float2  currentTexCoords = texCoords;
    float currentDepthMapValue = 1.0;
    currentDepthMapValue = tex2D(depthMap, currentTexCoords);

    [unroll(numLayers)] while (currentLayerDepth < currentDepthMapValue)
    {
        currentTexCoords -= deltaTexCoords;
        currentDepthMapValue = parallaxCenter - tex2D(depthMap, currentTexCoords).a; // Alpha channel of the normal map
