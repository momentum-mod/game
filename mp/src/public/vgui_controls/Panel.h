...
//
template <class T> inline T *SETUP_PANEL(T *panel)
{
...
//
//   example of use = to set the FG Color of a panel inside of a constructor (i.e before ApplySchemeSettings() has been
//   run on the child)
//
...
#define REGISTER_COLOR_AS_OVERRIDABLE(name, scriptname) AddToOverridableColors(&name, scriptname);
...
struct PanelSetting_t
{
    char name[128];
    PanelSettingType_t type;

    PanelSetting_t()
    {
        Q_strncpy(name, "", sizeof(name));
        type = TYPE_STRING;
    }

    PanelSetting_t(const char *pName, PanelSettingType_t t)
    {
...
#define BEGIN_PANEL_SETTINGS() PanelSetting_t settings[] = {
#define END_PANEL_SETTINGS() }; m_vecPanelSettings.AddMultipleToTail(sizeof(settings) / sizeof(PanelSetting_t), settings)
...
#define PANEL_ROUND_CORNER_ALL                                                                                         \
    PANEL_ROUND_CORNER_TOP_LEFT | PANEL_ROUND_CORNER_TOP_RIGHT | PANEL_ROUND_CORNER_BOTTOM_LEFT |                      \
        PANEL_ROUND_CORNER_BOTTOM_RIGHT
//=============================================================================
// HPE_END
//=============================================================================//-----------------------------------------------------------------------------
...
class Panel : public IClientPanel, virtual IForceVirtualInheritancePanel
{
  ...
  public:
    ...
    void MakeReadyForUse(); // fully construct this panel so its ready for use right now (i.e fonts loaded, colors set,
                            // default label text set, ...)
    ...
    // painting
    virtual VPANEL
    IsWithinTraverse(int x, int y,
                     bool traversePopups); // recursive; returns a pointer to the panel at those coordinates
    ...
    void MakePopup(
        ...
        bool disabled = false); // turns the panel into a popup window (ie. can draw outside of it's parents space)
    virtual void OnMove();
    ...
    template <class T> T *FindControl(const char *pszName, bool recurseDown = false)
    {
    ...
    virtual bool RequestInfo(KeyValues *outputData); // returns true if output is successfully written.  You should
                                                     // always chain back to the base class if info request is not
                                                     // handled
    ...
    virtual void SetSilentMode(
        bool bSilent); // change the panel's silent mode; if silent, the panel will not post any action signals

    // install a mouse handler
    virtual void
    InstallMouseHandler(Panel *pHandler); // mouse events will be send to handler panel instead of this panel
    ...
    void SetAutoResize(PinCorner_e pinCorner, AutoResize_e resizeDir, int nPinOffsetX, int nPinOffsetY,
    ...
    void PinToSibling(Panel *pSibling, PinCorner_e pinOurCorner, PinCorner_e pinSiblingCorner);
    ...
    virtual void SetPaintBackgroundType(int type); // 0 for normal(opaque), 1 for single texture from Texture1, and 2
                                                   // for rounded box w/ four corner textures
    ...
    // format: <type><whitespace | punctuation><keyname><whitespace| punctuation><type><whitespace |
    // punctuation><keyname>... unknown types as just displayed as strings in the UI (for future UI expansion)
    // DEPRECATED! Use InitSettings below!!!
    virtual const char *GetDescription() { return NULL;}

    // This void adds PanelSetting_t to the panel. Override this function to add your child panel's settings!!!
    // Feel free to utilize the BEGIN_PANEL_SETTINGS/END_PANEL_SETTINGS macros found in the header of the Panel class.
    // Make sure to call InitSettings() ------>>>>> BEFORE!!! <<<<<------ anything in your constructor!
    virtual void InitSettings();
    // Returns a list of the settings for this panel. Feel free to override this to return custom things, if needed.
    virtual CUtlVector<PanelSetting_t> &GetPanelSettings() { return m_vecPanelSettings; }
    ...
    virtual void OnMessage(const KeyValues *params,
                           VPANEL fromPanel);             // called when panel receives message; must chain back
    ...
    // If a panel implements this and returns true, then the IsKeyRebound check will fail and OnKeyCodeTyped messages
    // will pass through..
    ...
    virtual void DrawBoxFade(int x, int y, int wide, int tall, Color color, float normalizedAlpha, unsigned int alpha0,
                             unsigned int alpha1, bool bHorizontal, bool hollow = false);
    virtual void DrawHollowBox(int x, int y, int wide, int tall, Color color, float normalizedAlpha);
    //=============================================================================
    // HPE_BEGIN:
    //=============================================================================

    // [menglish] Draws a hollow box similar to the already existing draw hollow box function, but takes the indents as
    // params
    virtual void DrawHollowBox(int x, int y, int wide, int tall, Color color, float normalizedAlpha, int cornerWide,
                               int cornerTall);
    ...
    // Use this to prevent chaining up from a parent which can mess with mouse functionality if you don't want to chain
    // up from a child panel to the best
    //  draggable parent.
    ...
    template <class S> void PostMessageToAllSiblingsOfType(KeyValues *msg, float delaySeconds = 0.0f);
    ...
    // if set, Panel gets PerformLayout called after the camera and the renderer's m_matrixWorldToScreen has been setup,
    // so panels can be correctly attached to entities in the world
  ...
  protected:
    ...
    CUtlVector<PanelSetting_t> m_vecPanelSettings;

    // Proportionality scaling
    int GetScaledVal(int normalizedValue); // Gets the scaled value for this normalized value
    int GetNormalizedVal(int scaledValue); // Get the normalized value for a scaled value
  ...
  private:
    ...
    HCursor _cursor;
    CUtlFlags<int> _buildModeFlags; // flags that control how the build mode dialog handles this panel
    ...
    // Sibling pinning
    CUtlString _pinToSibling;      // string name of the sibling panel we're pinned to
  ...
  private:
    CUtlString _tooltipText; // Tool tip text for panels that share tooltip panels with other panels

    PHandle m_hMouseEventHandler;

    bool m_bWorldPositionCurrentFrame; // if set, Panel gets PerformLayout called after the camera and the renderer's
                                       // m_matrixWorldToScreen has been setup, so panels can be correctly attached to
    ...
    int m_nActionSignalLevel;
    bool m_bIgnoringScheme;
};
