      ...
      void CTriggerTimerStart::EndTouch(CBaseEntity *pOther)
{
    if (pOther->IsPlayer())
    {
        ...
        if (!pPlayer->m_bHasPracticeMode &&
            !g_Timer->IsRunning()) // do not start timer if player is in practice mode or it's already running.
        {
            if (IsLimitingSpeed())
            {
    ...
    if (pOther->IsPlayer())
    {
        ...
        if (g_Timer->IsRunning() && !pPlayer->IsWatchingReplay())
        {
            ...
            pPlayer->m_RunStats.SetZoneTime(zoneNum,
                                            g_Timer->GetCurrentTime() - pPlayer->m_RunStats.GetZoneEnterTime(zoneNum));
...
, DEFINE_OUTPUT(m_OnKeyPressed, "OnKeyPressed"), END_DATADESC()
