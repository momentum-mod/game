...
void Templates_ReconnectIOForGroup( CPointTemplate *pGroup )
{
	...
	if (pGroup->AllowNameFixup())
	{
		...
		for ( i = 0; i < iCount; i++ )
		{
			char val[MAPKEY_MAXLENGTH];

			if ( GroupTemplates[i].bChangeTargetname )
			{
				CEntityMapData *mapData = GroupTemplates[i].pMapDataParser;
				mapData->ExtractValue( "targetname", val );
				Q_strncat( val, ENTITYIO_FIXUP_STRING, sizeof(val), COPY_ALL_CHARACTERS );
				mapData->SetValue( "targetname", val );
			}
		}
	}
