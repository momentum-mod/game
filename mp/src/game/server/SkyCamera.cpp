...
BEGIN_DATADESC( CSkyCamera )
	...
	DEFINE_KEYFIELD( m_skyboxData.scale, FIELD_FLOAT, "scale" ),
...
{ 
	...
	m_skyboxData.area = engine->GetArea( GetAbsOrigin() );
...
void CSkyCamera::InputSetSkyColor( inputdata_t &inputdata ) { m_skyboxData.skycolor = inputdata.value.Color32(); }

void CSkyCamera::InputSetScale( inputdata_t& inputdata ) { m_skyboxData.scale = inputdata.value.Float(); }
