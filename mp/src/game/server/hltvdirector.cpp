...
void CHLTVDirector::StartBestPlayerCameraShot()
{
	...
	while( index != m_EventHistory.InvalidIndex() )
	{
		...
		{
			int eventIndex = dc.m_Event->GetInt("index");

			if (eventIndex < MAX_PLAYERS )
			{
				flPlayerRanking[eventIndex] += dc.m_Event->GetFloat("rank" );

				// find best camera
				if ( flPlayerRanking[eventIndex] > flBestRank )
				{
					iBestCamera = eventIndex;
					flBestRank = flPlayerRanking[eventIndex];
...
void CHLTVDirector::StartBestFixedCameraShot( bool bForce )
{
	...
	while( index != m_EventHistory.InvalidIndex() )
	{
		...
		{
			int eventIndex = dc.m_Event->GetInt("index"); 
			flCameraRanking[eventIndex] += dc.m_Event->GetFloat("rank" );

			// find best camera
			if ( flCameraRanking[eventIndex] > flBestRank )
			{
				iBestCamera = eventIndex;
				flBestRank = flCameraRanking[eventIndex];
				iBestTarget = dc.m_Event->GetInt("target"); 
			}
		}
...
bool CHLTVDirector::SetCameraMan( int iPlayerIndex )
{
	...
	{
		CBasePlayer *pPlayerRecip = UTIL_PlayerByIndex( i );

		if (pPlayerRecip && pPlayerRecip->GetTeamNumber() == TEAM_SPECTATOR && !pPlayerRecip->IsFakeClient() )
		{
			filter.AddRecipient(pPlayerRecip);
		}
	}
