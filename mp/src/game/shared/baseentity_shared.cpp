...
extern bool ParseKeyvalue(void *pObject, typedescription_t *pFields, int iNumFields, const char *szKeyName,
                          const char *szValue);
extern bool ExtractKeyvalue(void *pObject, typedescription_t *pFields, int iNumFields, const char *szKeyName,
                            char *szValue, int iMaxLen);
#endif
...
float k_flMaxEntityEulerAngle =
    360.0 *
    1000.0f; // really should be restricted to +/-180, but some code doesn't adhere to this.  let's just trap NANs, etc
...
// position of eyes
Vector CBaseEntity::EyePosition(void) { return GetAbsOrigin() + GetViewOffset(); }

const QAngle &CBaseEntity::EyeAngles(void) { return GetAbsAngles(); }

const QAngle &CBaseEntity::LocalEyeAngles(void) { return GetLocalAngles(); }

// position of ears
Vector CBaseEntity::EarPosition(void) { return EyePosition(); }

void CBaseEntity::SetViewOffset(const Vector &v) { m_vecViewOffset = v; }

const Vector &CBaseEntity::GetViewOffset() const { return m_vecViewOffset; }

//-----------------------------------------------------------------------------
// center point of entity
//-----------------------------------------------------------------------------
const Vector &CBaseEntity::WorldSpaceCenter() const { return CollisionProp()->WorldSpaceCenter(); }

#if !defined(CLIENT_DLL)
#define CHANGE_FLAGS(flags, newFlags)                                                                                  \
    {                                                                                                                  \
        unsigned int old = flags;                                                                                      \
        flags = (newFlags);                                                                                            \
        gEntList.ReportEntityFlagsChanged(this, old, flags);                                                           \
    }
#else
#define CHANGE_FLAGS(flags, newFlags) (flags = (newFlags))
#endif

void CBaseEntity::AddFlag(int flags) { CHANGE_FLAGS(m_fFlags, m_fFlags | flags); }

void CBaseEntity::RemoveFlag(int flagsToRemove) { CHANGE_FLAGS(m_fFlags, m_fFlags & ~flagsToRemove); }

void CBaseEntity::ClearFlags(void) { CHANGE_FLAGS(m_fFlags, 0); }

void CBaseEntity::ToggleFlag(int flagToToggle) { CHANGE_FLAGS(m_fFlags, m_fFlags ^ flagToToggle); }
...
bool CBaseEntity::BlocksLOS(void) { return !IsEFlagSet(EFL_DONTBLOCKLOS); }
...
bool CBaseEntity::IsAIWalkable(void) { return !IsEFlagSet(EFL_DONTWALKON); }
...
bool CBaseEntity::KeyValue(const char *szKeyName, const char *szValue)
{
    ...
    else
    {
        ...
        for (datamap_t *dmap = GetDataDescMap(); dmap != NULL; dmap = dmap->baseMap)
        {
            if (!printKeyHits && *ent_debugkeys.GetString() &&
                !Q_stricmp(dmap->dataClassName, ent_debugkeys.GetString()))
            {
...
//-----------------------------------------------------------------------------
IPhysicsObject *CBaseEntity::VPhysicsInitNormal(SolidType_t solidType, int nSolidFlags, bool createAsleep,
                                                solid_t *pSolid)
{
    if (!VPhysicsInitSetup())
        return NULL;
...
IPhysicsObject *CBaseEntity::VPhysicsInitShadow(bool allowPhysicsMovement, bool allowPhysicsRotation, solid_t *pSolid)
{
    ...
    {
        pPhysicsObject =
            PhysModelCreateOBB(this, CollisionProp()->OBBMins(), CollisionProp()->OBBMaxs(), origin, angles, false);
    }
    else
    {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CBaseEntity::CreateVPhysics() { return false; }
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CBaseEntity::IsPrecacheAllowed() { return m_bAllowPrecache; }

//-----------------------------------------------------------------------------
// Purpose: static method
// Input  : allow -
//-----------------------------------------------------------------------------
void CBaseEntity::SetAllowPrecache(bool allow) { m_bAllowPrecache = allow; }
...
void CBaseEntity::FireBullets(const FireBulletsInfo_t &info)
{
    ...
    traceFilter.SetPassEntity(this); // Standard pass entity for THIS so that it can be easily removed from the list
                                     // after passing through a portal
    ...
    for (int iShot = 0; iShot < info.m_iShots; iShot++)
    {
        ...
        if (bStartedInWater)
        {
            ...
            if (pShootThroughPortal)
            {
                ...
                UTIL_Portal_PointTransform(pShootThroughPortal->MatrixThisToLinked(), vBubbleEnd,
                                           vTransformedIntersection);
        ...
        if (tr.fraction != 1.0)
        {
            ...
            int soundEntChannel = (info.m_nFlags & FIRE_BULLETS_TEMPORARY_DANGER_SOUND) ? SOUNDENT_CHANNEL_BULLET_IMPACT
            ...
            if (iPlayerDamage)
            {
                ...
                {
                    if (tr.m_pEnt->GetServerVehicle()->GetPassenger() &&
                        tr.m_pEnt->GetServerVehicle()->GetPassenger()->IsPlayer())
                    {
                        flActualDamage = iPlayerDamage;
                    }
                }
        ...
        if (tr.m_pEnt != NULL)
        {
            ...
            if ((psurf != NULL) && (psurf->game.material == CHAR_TEX_GLASS) &&
                (tr.m_pEnt->ClassMatches("func_breakable")))
            {
        ...
        if ((info.m_iTracerFreq != 0) && (tracerCount++ % info.m_iTracerFreq) == 0 && (bHitGlass == false))
        {
            if (bDoServerEffects == true)
            {
                ...
                {
                    Vector vTransformedIntersection;
                    UTIL_Portal_PointTransform(pShootThroughPortal->MatrixThisToLinked(), Tracer.endpos,
                                               vTransformedIntersection);
                    ...
                    UTIL_Portal_PointTransform(pShootThroughPortal->MatrixThisToLinked(), info.m_vecSrc,
        ...
        // NOTENOTE: We could expand this to a more general solution for various material penetration types (wood, thin
        // metal, etc)
    ...
    {
        TE_HL2MPFireBullets(entindex(), tr.startpos, info.m_vecDirShooting, info.m_iAmmoType, iEffectSeed,
                            info.m_iShots, info.m_vecSpread.x, bDoTracers, bDoImpacts);
    }
#endif
...
//-----------------------------------------------------------------------------
bool CBaseEntity::HandleShotImpactingWater(const FireBulletsInfo_t &info, const Vector &vecEnd,
                                           ITraceFilter *pTraceFilter, Vector *pVecTracerDest)
{
    trace_t waterTrace;
    ...
    if ((enginetrace->GetPointContents(waterTrace.endpos - Vector(0, 0, 0.1f)) & (CONTENTS_WATER | CONTENTS_SLIME)) ==
        0)
        return false;
...
ITraceFilter *CBaseEntity::GetBeamTraceFilter(void) { return NULL; }
...
int CBaseEntity::BloodColor() { return DONT_BLEED; }
...
void CBaseEntity::TraceBleed(float flDamage, const Vector &vecDir, trace_t *ptr, int bitsDamageType)
{
    ...
    for (i = 0; i < cCount; i++)
    {
        ...
        AI_TraceLine(ptr->endpos, ptr->endpos + vecTraceDir * -flTraceDist, MASK_SOLID_BRUSHONLY & ~CONTENTS_GRATE,
                     this, COLLISION_GROUP_NONE, &Bloodtr);
...
const char *CBaseEntity::GetTracerType() { return NULL; }
...
void CBaseEntity::ApplyLocalVelocityImpulse(const Vector &inVecImpulse)
{
    ...
    if (inVecImpulse != vec3_origin)
    {
        ...
        case 0:
            if (CheckEmitReasonablePhysicsSpew())
            {
                Warning("Clamping ApplyLocalVelocityImpulse(%f,%f,%f) on %s\n", inVecImpulse.x, inVecImpulse.y,
                        inVecImpulse.z, GetDebugName());
            }
            break;
        }
...
void MarkEntitiesAsTouching(IClientEntity *e1, IClientEntity *e2)
{
...
void CM_WorldSpaceBounds(ICollideable *pCollideable, Vector *pMins, Vector *pMaxs)
{
    ...
    {
        TransformAABB(pCollideable->CollisionToWorldTransform(), pCollideable->OBBMins(), pCollideable->OBBMaxs(),
                      *pMins, *pMaxs);
    }
}

uint GetRequiredTriggerFlags(ICollideable *col)
{
    // debris only touches certain triggers
    if (col->GetCollisionGroup() == COLLISION_GROUP_DEBRIS)
        return FSOLID_TRIGGER_TOUCH_DEBRIS;

    // triggers don't touch other triggers (might be solid to other ents as well as trigger)
    if ((col->GetSolidFlags() & FSOLID_TRIGGER) != 0)
        return 0;
...
{
  public:
    CTouchLinks_ClientSide(IClientEntity *pEnt, const Vector *pPrevAbsOrigin, bool accurateBboxTriggerChecks)
        : m_TouchedEntities(8, 8)
    {
        m_pEnt = pEnt;
        m_pCollide = pEnt->GetCollideable();
        m_nRequiredTriggerFlags = GetRequiredTriggerFlags(m_pCollide);
        Assert(m_pCollide);
...
class CTriggerMoved_ClientSide : public IPartitionEnumerator
{
  public:
    ...
    void TriggerMoved(IClientEntity *pTriggerEntity)
    {
        ...
        partition->EnumerateElementsInBox(PARTITION_CLIENT_SOLID_EDICTS, vecAbsMins, vecAbsMaxs, false, this);
    }
    ...
    IterationRetval_t EnumElement(IHandleEntity *pHandleEntity)
    {
        ...
        int nReqFlags = GetRequiredTriggerFlags(pTouchCollide);
...
void CBaseEntity::PhysicsTouchTriggers(const Vector *pPrevAbsOrigin)
{
    ...
    if (pEntity && !IsWorld())
    {
        ...
        bool isSolidCheckTriggers =
            IsSolid() &&
            !isTriggerCheckSolids; // NOTE: Moving triggers (items, ammo etc) are not
        ...
        {
#ifdef CLIENT_DLL
            ICollideable *pSolidCollide = CollisionProp();
            if (!pPrevAbsOrigin)
            {
                CTouchLinks_ClientSide touchEnumerator(pEntity, NULL, true);
        ...
        {
#ifdef CLIENT_DLL
            CTriggerMoved_ClientSide triggerEnum(true);
            triggerEnum.TriggerMoved(pEntity);
...
void CBaseEntity::CollisionRulesChanged()
{
    ...
    // that can change the state that a collision filter will return (like m_Solid) needs to call
...
ConVar sv_alternateticks("sv_alternateticks", (IsX360()) ? "1" : "0", FCVAR_SPONLY,
