...
void CMomentumGameMovement::DFStepSlideMove(bool inAir)
{
    ...
    if (sv_slidestyle.GetInt() == 1)
    {
        ...
        if (trace.fraction < 1.0)
        {
            ...
            else if (trace.endpos.z > startOrigin.z + (startVel.z * gpGlobals->frametime)
                && startVel.z > 0 && !mv->m_bRampSliding && !(mv->m_nButtons & IN_JUMP))
            {
                if ((sv_jumpstyle.GetFloat() == 2 || sv_jumpstyle.GetFloat() == 4) &&
                    gpGlobals->curtime - m_pPlayer->m_Data.m_flLastJumpTime < sv_jump_buffertime.GetFloat())
                {
    ...
    if (mv->m_flWallClipTime > gpGlobals->curtime &&
        oldSpeed > mv->m_vecVelocity.Length2D())
    {
        VectorCopy(startVel, mv->m_vecVelocity);
    }

    if (trace.plane.normal[2] >= 0.7 && (sv_rampslide.GetInt() == 2 || (sv_rampslide.GetInt() == 1 && m_pPlayer->m_CurrentSlideTrigger != nullptr))
        && oldSpeed > mv->m_vecVelocity.Length2D())
    {
