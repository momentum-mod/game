...
{
    float flForwardSpeed =
        RemapValClamped((gpGlobals->curtime - m_flChargeBeginTime), 0.0f, 4.0f,
                        900, 2400);
...
bool CMomentumPipebombLauncher::DetonateRemotePipebombs(bool bFizzle)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (pTemp)
        {
            ...
            if (bFizzle == false)
            {
                if ((gpGlobals->curtime - /*pTemp->m_flCreationTime*/ 0.5f /*placeholder value because of an error*/) < 0.8f) //Pipebomb arm time
                {
