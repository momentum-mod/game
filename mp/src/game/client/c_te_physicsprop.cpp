...
void TE_PhysicsProp( IRecipientFilter& filter, float delay,
	int modelindex, int skin, const Vector& pos, const QAngle &angles, const Vector& vel, bool breakmodel, int effect )
{
	tempents->PhysicsProp( modelindex, skin, pos, angles, vel, breakmodel, effect );
	RecordPhysicsProp( pos, angles, vel, modelindex, breakmodel, skin, effect );
}
