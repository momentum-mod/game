...
// Only test this in tf2
	...
	return m_pModel && !IsEffectActive(EF_NODRAW) && (m_index != 0);
}
...
void C_BaseEntity::CheckInitPredictable( const char *context )
{
	...
	{
		if ( m_PredictableID.IsActive() &&
			( player->m_index - 1 ) == m_PredictableID.GetPlayer() )
		{
...
void C_BaseEntity::CreateLightEffects( void )
{
	...
	if ( m_index == render->GetViewEntity() )
		return;
...
{
	if ( render->GetViewEntity() == m_index )
		return true;
...
void C_BaseEntity::ComputeFxBlend( void )
{
	...
	offset = m_index * 363.0;// Use ent index to de-sync these fx
									  ...
									  int decalIndex, bool doTrace, trace_t& tr )
{
	...
	effects->DecalShoot( decalIndex, m_index, 
                        m_pModel, GetAbsOrigin(), GetAbsAngles(), decalCenter, 0, 0 );
}
	...
	case mod_brush:
		{
			...
			effects->DecalColorShoot( decalIndex, m_index, m_pModel, GetAbsOrigin(), GetAbsAngles(), decalCenter, 0, nFlags, cColor32 );
		}
		break;
