	...
	const Vector* org, const Vector* direction, int r, int g, int b, int a, int amount )
{
	...
	for (count = 0 ; count < (amount/5); count ++)
	{
		...
		if(p)
		{
			...
			{
				StandardParticle_t *pParticle = pRen->AddParticle();
				if(pParticle)
				{
					pRen->SetParticleLifetime(pParticle, 3);
					pParticle->SetColor(random->RandomFloat(0.7, 1.0), g, b);
					pParticle->SetAlpha(a);
					pParticle->m_Pos.Init(
						(*org)[0] + random->RandomFloat(-1,1),
						(*org)[1] + random->RandomFloat(-1,1),
						(*org)[2] + random->RandomFloat(-1,1));
					
					pRen->SetParticleType(pParticle, pt_vox_slowgrav);
					...
					pParticle->m_Velocity = dirCopy * speedCopy;
				}
			}
		}
	}
