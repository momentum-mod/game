...
SavelocReqFrame::SavelocReqFrame() : BaseClass(nullptr, "SavelocReqFrame"), m_iSteamID(0)
{
    ...
    m_pStatusLabel = new Label(this, "StatusLabel", "#MOM_Saveloc_Frame_Obtaining");
...
{
    m_iSteamID = 0;
    m_pStatusLabel->SetText("#MOM_Saveloc_Frame_Obtaining");
    BaseClass::Close();
}
...
void SavelocReqFrame::FireEvent(KeyValues* pKv)
{
    ...
    {
        m_pStatusLabel->SetText("#MOM_Saveloc_Frame_Downloaded");
    ...
    {
        m_pStatusLabel->SetText("#MOM_Saveloc_Frame_Requester_Left");
        m_pRequestButton->SetEnabled(false);
        m_pSavelocSelect->SetEnabled(false);
        m_pToggleAllButton->SetEnabled(false);
    }
...
void SavelocReqFrame::OnCommand(const char* command)
{
    if (FStrEq(command, "Request"))
    {
        if (m_iSteamID)
        {
            ...
            // Let em know
            m_pStatusLabel->SetText("#MOM_Saveloc_Frame_Downloading");
        }
        else
        {
...
void SavelocReqFrame::SetSavelocCount(int count)
{
    ...
    m_pStatusLabel->SetText("#MOM_Saveloc_Frame_Select");
