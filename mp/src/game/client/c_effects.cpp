...
void CSnowFallManager::FindSnowVolumes( Vector &vecCenter, float flRadius, Vector &vecEyePos, Vector &vecForward )
{
	...
	if ( r_SnowDebugBox.GetFloat() != 0.0f )
	{
		...
		{
			Vector vecCenter2, vecMin, vecMax;
			vecCenter2 = ( m_aSnow[iSnow].m_vecMin, m_aSnow[iSnow].m_vecMax ) * 0.5;
			vecMin = m_aSnow[iSnow].m_vecMin - vecCenter2;
			vecMax = m_aSnow[iSnow].m_vecMax - vecCenter2;
			debugoverlay->AddBoxOverlay( vecCenter2, vecMin, vecMax, QAngle( 0, 0, 0 ), 200, 0, 0, 25, r_SnowDebugBox.GetFloat() );
		}
	}
#endif
}
