...
public:
	...
	bool IsValidFor( const Vector& origin ) const
	{
		return m_radius > 0 && m_origin.DistToSqr( origin ) < 1.0f;
	}
...
//-----------------------------------------------------------------------------
void C_BeamSpotLight::ClientThink()
{
	...
	m_flSpotlightCurLength = ( 0.6f * m_flSpotlightCurLength ) + ( 0.4f * flBeamLength );
...
//------------------------------------------------------------------------------
void C_BeamSpotLight::SpotlightCreate()
{
...
//------------------------------------------------------------------------------
void C_BeamSpotLight::SpotlightDestroy()
{
	if ( m_hSpotlight )
	{
...
void C_BeamSpotLight::ComputeRenderInfo()
{
	...
	{
		SetRenderColorA( 255 * ( 1 - ( ( m_flSpotlightCurLength - m_flSpotlightMaxLength ) / m_flSpotlightMaxLength ) ) );
		m_hSpotlight->SetFadeLength( m_flSpotlightMaxLength );
	}
	else
	{
